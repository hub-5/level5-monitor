...
Los avisos de Discord no dicen solo "esta página cambió": cada página con
novedades aparece como su propia tarjeta (lo que Discord llama un *embed*),
con el título real de la página (el que se ve en la pestaña del navegador,
sin tener que abrir el enlace), el enlace en sí, y la diferencia exacta,
línea a línea, entre la versión anterior y la nueva. Si la página tiene
alguna imagen nueva (artwork, una captura de pantalla recién añadida), se
muestra incrustada directamente en la tarjeta. Tres tipos de aviso:

- **🔄 Modificada** — la página ya existía y su contenido visible ha
  cambiado. La tarjeta muestra las líneas quitadas (➖) y las añadidas (➕).
  Por ejemplo, si LEVEL-5 cambia la fecha de lanzamiento de un juego, verás
  algo como:
  ...
  El diff se recorta a un número razonable de líneas para no desbordar la
  tarjeta; si hay más cambios de los que se muestran, se indica cuántas
  líneas más hay. Si además aparece una imagen que no estaba antes en la
  página, se adjunta; si la página no tiene ninguna imagen nueva (solo
  cambió texto), la tarjeta no lleva imagen.
- **🆕 Nueva página** — una URL que no existía antes en ese sitio (por
  ejemplo, la web de TGS2026 activándose). La tarjeta incluye un fragmento
  del contenido y, si la página tiene alguna imagen (normalmente la que usa
  para compartir en redes sociales), se muestra también.
...
Si en una misma comprobación cambian muchas páginas a la vez, los avisos se
reparten en varios mensajes de Discord (máximo 5 tarjetas por mensaje) para
no desbordar los límites de la plataforma.
...
migra en silencio su formato interno de almacenamiento — **no vas a recibir
un aluvión de avisos de "modificada" por este cambio**, igual que no lo hubo
al aplicar las correcciones anteriores. Solo a partir de ese momento, el
siguiente cambio real que ocurra en cada página generará ya un diff exacto.
...
- **`state.json` solo crece (y ahora pesa más)**: las páginas descubiertas
  ...
  borra). Además, ahora se guarda el texto completo de cada página y la
  lista de imágenes que contiene, no solo un hash, para poder mostrar diffs
  exactos e imágenes nuevas — así que `state.json` pesará notablemente más
  que antes. No causa ningún problema de funcionamiento (GitHub no tiene un
  límite práctico relevante para esto), solo ocupa más espacio en el
  repositorio con el tiempo.
- **Si el propio monitor falla, te avisa por Discord aparte**: además de
  los avisos de cambios, si `monitor.py` casca o no se consigue guardar
  `state.json` tras varios intentos, el workflow manda un mensaje aparte
  ("⚠️ El LEVEL-5 Website Monitor ha fallado...") con el enlace directo al
  log de esa ejecución. Si el fallo persiste en ejecuciones consecutivas
  (por ejemplo, una web caída durante horas), recibirás ese aviso cada vez
  que el cron vuelva a fallar — no hay, de momento, un "silenciar tras el
  primer aviso" para este caso concreto.
